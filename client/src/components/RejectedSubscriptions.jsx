...
import groupSubscriptions from '../utils/groupSubscriptions'; // Import the utility function

const RejectedSubscriptions = ({ transactions }) => {
    const [confirmed, setConfirmed] = useState([]);
    const [loading, setLoading] = useState(true);
    const [expandedGroups, setExpandedGroups] = useState({});

    useEffect(() => {
        fetch('http://localhost:2000/rejected')
            .then(res => res.json())
            .then(data => {
                setConfirmed(data);
                setLoading(false);
    ...
    return (
        <div>
            <h2>Rejected Subscriptions</h2>
            {loading ? (
                <p>Loading rejected subscriptions...</p>
            ) : groupSubscriptions(transactions).length === 0 ? (
                <p>No rejected subscriptions found.</p>
            ) : (
